...
本次仅调整手机布局与输入提示，保留原 localStorage key、数据结构、历史、摘要和训练逻辑。600 px 及以下：睡眠字段单栏、训练体感 2×2、重量/次数各占一行、顶部更紧凑；360 px 及以下概览日期与健身房改为单栏。评分和 RIR 仍为一行五个按钮。

空的时间字段不会自动保存系统时间。WebKit 可能在空时间框中绘制示例时间；本版在手机端未聚焦时隐藏此显示，点击后仍使用原生选择器。已经保存的时间（包括 18:04）继续保留，如不需要，请在输入框中手动清空。

### GitHub Pages 更新后让 iPhone 获取新界面

1. 上传新版 `styles.css`、`app.js`、`sw.js`，等待 GitHub Pages 部署成功。本版缓存名已从 `haoxuan-shell-v1` 升级为 `haoxuan-shell-v2`。
2. 保持联网，打开原来的主屏幕 App，等待几秒，让浏览器检查并下载新版 Service Worker 与资源。
3. 关闭 Safari 中这个站点的所有标签页，并从 App 切换器划掉主屏幕 App。旧窗口全部关闭后，等待中的新版 Service Worker 才能激活。
4. 重新从原来的主屏幕图标打开；进入“睡眠与精神”，手机上应看到单栏输入。
