...
## 2026-09-06 C 日计划（shell v5）

首次打开新版时，在原 `haoxuan-training-log:v1` 中安全加入 `2026-09-06_C`，健身房为 `Anytime成増`。六个动作、15个正式组，建议重量依次为42.5、45、60、55、32.5、37.5 kg。全部为机器显示重量，每组仍可独立修改。

只在目标记录不存在，或没有编辑时间、完成状态、反馈和逐组实际输入时初始化；任何修改过的重量、次数、RIR、体感、反馈或完成状态都会触发保留原记录。冲突时显示提示，仍更新未来使用的 C 模板。历史 B/C 和睡眠不变，不补写昨天完成状态。

指导说明使用当天记录已有的 `feedback.notes` 字段，因此可以在原备注区查看、编辑，也会按原摘要格式包含在备注中。热身不计入15个正式组。未来 C 日使用六动作模板；历史训练快照不随模板修改。

主数据 key 和 schema 不变。独立的一次性标记 `haoxuan-training-log:plan:2026-09-06-C` 防止后续启动重新覆盖模板或记录；不要手动删除该标记。代码不会读取其他设备数据，需在保存训练的原 Safari/PWA 入口打开新版，才能检查该入口已有记录。

验证脚本：`tests/c-plan.cjs`（需要 Playwright）。覆盖缺失、空记录、未填写预设、重量/次数/RIR0/体感/完成/备注/编辑时间冲突、15组、独立重量修改、旧历史与睡眠保留、模板后续修改和重复启动。
